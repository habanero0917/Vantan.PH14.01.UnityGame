...
public class EnemyCtrl : MonoBehaviour {
	...
	void Attacking()
	{
        ...
        waitTime = Random.Range(waitBaseTime, waitBaseTime * 5.0f);
        // ターゲットをリセットする
        attackTarget = null;
    }
    ...
    void Died()
	{
        ...
        dropItem();
        Destroy(gameObject);
		AudioSource.PlayClipAtPoint (deathSeClipe, transform.position);
//		TimeScript timeme = GetComponent<TimeScript> ();
//		timeme.TimeUp ();
		TimeScript timeme = GetComponent<TimeScript>();
		if(timeme!=null){
			timeme.TimeUp();
		} else {
			Debug.Log ("set Script TimeUp!");
